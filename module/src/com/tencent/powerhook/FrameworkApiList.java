...
public class FrameworkApiList {
	 ...
	 * 该原生方法之前调用你自己的方法
	 */
	public static final HashMap<String,List<PlugMethod>> mInvokeBeforeOriMethods; 
	/**
	 * 该原生方法调用之后执行自己的方法
	 */
	public static final HashMap<String,List<PlugMethod>> mInvokeAfterOriMethods;
	
	
	private static FrameworkApiList mInstance;
	
	static{
		mInvokeBeforeOriMethods = new HashMap<String,List<PlugMethod>>();
		mInvokeAfterOriMethods = new HashMap<String,List<PlugMethod>>();
	}
	...
	public static synchronized void putHookMethod(int minVersion,int maxVersion,int invokeType,String className,String methodName,boolean isConstructor,Class<?>... argsType){
		List<PlugMethod> methodList = null;
		switch (invokeType) {
		...
		if(methodList == null){
			methodList = new ArrayList<PlugMethod>();
		}
		PlugMethod method = new PlugMethod();
	...
	@Deprecated
	public static  List<PlugMethod> getHookMethods(String className){
		return null;
	}
	
	
	public static class PlugMethod{
